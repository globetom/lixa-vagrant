...
if [ ${has} -eq 0 ]
then
sudo yum install -y https://download.postgresql.org/pub/repos/yum/9.6/redhat/rhel-6-x86_64/pgdg-centos96-9.6-3.noarch.rpm
...
sudo yum install -y gcc glib2-devel libxml2-devel libuuid-devel autoconf net-tools
...
sudo -u vagrant ./configure --enable-debug --enable-crash --with-postgresql=/usr/pgsql-9.6/bin/pg_config
sudo -u vagrant make
